...
// Results tag switching functionality
function showResults(tag) {
    ...
    const allTags = document.querySelectorAll('.result-tag');
    allTags.forEach(tagElement => {
        tagElement.classList.remove('is-active');
    });
    
    // Show selected content
    const selectedContent = document.getElementById('results-' + tag);
    if (selectedContent) {
        selectedContent.style.display = 'block';
    }
    
    // Add active class to clicked tag
    const clickedTag = document.querySelector(`[data-tag="${tag}"]`);
    if (clickedTag) {
